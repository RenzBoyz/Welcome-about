...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('RenzBoyz', {
		font: 'Standard',
...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	kurr.browserDescription = [ 'RenzBoyz', 'ubuntu', '3.0' ]
...
kurr.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6281393526422@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				teks = `🌹 Hi @${num.split('@')[0]} 👋\n🌹 Selamat Datang Di Group: ${mdata.subject}\n\n Join Pada Jam: ${time} Waktu Server\n\nJoin My Gc:\nhttps://chat.whatsapp.com/FOmeta4o0xWKn4tggb9RU5`

				sendButImage(anu.jid, teks, `RenzBoyz`, img,but = [{buttonId:`p`, 

               buttonText:{displayText: 'Intro Dulu🛐'},type:1},{buttonId:`pr`, 

               buttonText:{displayText: 'Semoga Betah🙏'},type:1},{buttonId:`pusanr`, 

               buttonText:{displayText: 'Salam Kenal🤝'},type:1}], options = {contextInfo: {mentionedJid: [num, htod]}})
			...
			try {
				...
				sendButImage(anu.jid, teks, `RenzBoyz`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `SAYONARA !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
kurr.on('group-update', async (anu) => {
    ...
    kurr.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6281393526422@s.whatsapp.net","6281393526422@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
